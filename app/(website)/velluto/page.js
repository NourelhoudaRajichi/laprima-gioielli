...
async function getProducts() {
  try {
    ...
    console.log(`Velluto products found: ${filtered.length}`);
    return filtered.length ? filtered : null;
  } catch (e) {
    console.error("Velluto fetch error:", e.message);
    return null;
  }
}

export default async function VellutoPage() {
  const wcProducts = await getProducts();
  return <Velluto wcProducts={wcProducts} />;
}
