...
export default function VipPage({ products: rawProducts, allCats }) {
    ...
    <div style={{ background:"#fff", minHeight:"100vh" }}>
      ...
      <div style={{ maxWidth:1400, margin:"0 auto", padding:"48px 24px 80px", display:"flex", gap:32, alignItems:"flex-start" }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <p style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:14, color:navy, opacity:0.5, marginBottom:24 }}>
            ...
            <>
                      ...
                      <Link
                        ...
                        <img src={img} alt={product.name}
                          style={{ width:"100%", height:"100%", objectFit:"cover",
                            transition:"transform 0.5s ease", transform: isHov ? "scale(1.1)" : "scale(1)" }} />
                      </Link>
                          ...
                          <>
                            ...
                            <div className="lpg-cart-actions" style={{ position:"absolute", inset:0,
                              ...
                              <button onClick={() => handleToggleWishlist(product)}
                                ...
                                <Heart size={14} style={{ color: inWish ? pink : navy, fill: inWish ? pink : "none" }} />
                              </button>
                            </div>
                          </>
                        ) : (
              ...
              {totalPages > 1 && (
                <div style={{ display:"flex", justifyContent:"center", gap:8, marginTop:48, flexWrap:"wrap" }}>
                  {page > 1 && <button onClick={() => { setPage(p=>p-1); window.scrollTo({top:0,behavior:"smooth"}); }} style={pageBtnStyle(false)}>← Prev</button>}
                  {Array.from({ length: totalPages }, (_,i) => i+1).map(n => (
                    <button key={n} onClick={() => { setPage(n); window.scrollTo({top:0,behavior:"smooth"}); }} style={pageBtnStyle(n===page)}>{n}</button>
                  ))}
                  {page < totalPages && <button onClick={() => { setPage(p=>p+1); window.scrollTo({top:0,behavior:"smooth"}); }} style={pageBtnStyle(false)}>Next →</button>}
                </div>
              )}
            </>
          )}
      ...
      <style>{`
        .lpg-agent-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:24px; }
        .lpg-agent-card { display:flex; flex-direction:column; align-items:center; text-align:center; cursor:pointer; }
        ...
        @media (max-width:1024px) { .lpg-agent-grid { grid-template-columns:repeat(2,1fr) !important; } .lpg-agent-sidebar { display:none !important; } }
        @media (max-width:600px)  { .lpg-agent-grid { grid-template-columns:repeat(1,1fr) !important; } }
      `}</style>
    </div>
  );
}
...
const pageBtnStyle = (active) => ({
  ...
  background: active ? pink : "#fff", color: active ? "#fff" : pink,
